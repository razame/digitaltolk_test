...
class BookingRepository extends BaseRepository
{
    ...
    public function store($user, $data)
    {
        ...
        if ($data['immediate'] == 'no' && isset($data['due_date']) && $data['due_date'] == '') {
            return [
                'status' => 'fail',
                'message' => "Du måste fylla in alla fält",
                'field_name' => "due_date"
        ...
        if ($data['immediate'] == 'no' && isset($data['due_time']) && $data['due_time'] == '') {
            return [
                'status' => 'fail',
                'message' => "Du måste fylla in alla fält",
                'field_name' => "due_time"
        ...
        if ($data['immediate'] == 'no' && !isset($data['customer_phone_type']) && !isset($data['customer_physical_type'])) {
            return [
                'status' => 'fail',
    ...
    public function storeJobEmail($data)
    {
        ...
        $is_user_email_empty = empty($job->user_email);

        $email = !$is_user_email_empty ? $job->user_email : $user->email;
        $name = $user->name;
    ...
    public function jobToData($job)
    {
        ...
        $attributes = [
            'from_language_id', 'immediate', 'duration', 'status', 'gender', 'certified',
            'due', 'job_type', 'customer_phone_type', 'customer_physical_type', 'customer_type',
            'job_id' =>'id', 'customer_town' => 'town'
        ];

        foreach($attributes as $key => $attribute){
            $data[
                is_integer($key) ? $attribute : $key
            ] = $job->{$attribute};
        }
        ...
        $data['job_for'][] = $pronounsMapper[$job->gender];
        ...
        if($job->certified != null)
            $data['job_for'] = array_merge($data['job_for'], $certificationMapper[$job->certified]);
    ...
    public function jobEnd($post_data = [])
    {
        ...
        $interval = $completedDate->diffAsCarbonInterval($dueDate)->forHumans();

        $this->updateJobDetails($job, $completedDate, $interval);

        $this->sendEmail($job->user, $job, $interval, 'faktura');
        $this->sendEmail($job->translatorJobRel->user, $job, $interval, 'lön');

        Event::fire(new SessionEnded($job, ($post_data['userid'] == $job->user_id) ? $job->translatorJobRel->user_id : $job->user_id));

        $this->updateTranslatorDetails($job->translatorJobRel, $completedDate, $post_data['userid']);
    }

    protected function updateJobDetails(Job $job, $completedDate, $interval)
    {
        $job->update([
    ...
    protected function sendEmail($user, Job $job, $sessionTime, $forText)
    {
    ...
    protected function updateTranslatorDetails($translatorJobRel, $completedDate, $userId)
    {
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $translatorToJobMapper = [
            'professional' => 'paid', 'rwstranslator' => 'rws', 'volunteer' => 'unpaid'
        ];
        $languages = UserLanguages::where('user_id', '=', $user_id)->get()->pluck('lang_id')->all();
        $job_ids = Job::getJobs($user_id, $translatorToJobMapper[$translator_type], 'pending', $languages, $user_meta->gender, $user_meta->translator_level);
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $delay_push_translator_array = array();     // suitable translators (need to delay push)

        foreach ($users as $oneUser) {
            $this->populateTranslatorsArrays($oneUser, $exclude_user_id, $data, $job, $translator_array, $delay_push_translator_array);
        }
        ...
        $logger->addInfo('Push send for job ' . $job->id, [$translator_array, $delay_push_translator_array, $msg_text, $data]);
        ...
        $this->sendPushNotificationToSpecificUsers($delay_push_translator_array, $job->id, $data, $msg_text, true); // send new booking push to suitable translators(need to delay)
    }

    // I could not assign a proper name for this method
    // because I have a little domain knowledge
    private function populateTranslatorsArrays($oneUser, $exclude_user_id, $data, $job, &$translator_array, &$delay_push_translator_array){
        ...
        foreach ($jobs as $oneJob) {
            $this->populate($job, $oneJob, $oneUser,$delay_push_translator_array,$translator_array);
        }
    }

    private function populate($job, $oneJob, $oneUser, &$delay_push_translator_array, &$translator_array){
        if ($job->id != $oneJob->id) {
            return;
        }
        ...
        if ($this->isNeedToDelayPush($oneUser->id)) {
            $delay_push_translator_array[] = $oneUser;
        } else {
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        if ($job->customer_physical_type == 'yes' && $job->customer_phone_type == 'no') {
            ...
            || ($job->customer_physical_type == 'yes' && $job->customer_phone_type == 'yes')
        ) {
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        $env = config('app.env') == 'prod' ? 'prod' : 'dev';
        $onesignalAppID = config('app.'.$env.'_one_signal_app_id');
        $onesignalRestAuthKey = sprintf("Authorization: Basic %s", config('app.'.$env.'_one_signal_api_key'));
...
//        }
    ...
    public function sendNotificationByAdminCancelJob($job_id)
    {
        ...
        if ($job->certified != null && $job->certified == 'both') {
            $data['job_for'][] = 'normal';
            $data['job_for'][] = 'certified';
        } else if ($job->certified != null && $job->certified == 'yes') {
            $data['job_for'][] = 'certified';
        }
    ...
    public function cancelJobAjax($data, $user)
    {
        ...
        */
        ...
        if ($cUser->is('customer')) {
            $response = $this->cancelCustomerJob($job, $translator, $job_id, $response);
        } else {
            $response = $this->cancelNonCustomerJob($job, $translator, $job_id, $response);

        }
        return $response;
    }

    private function cancelNonCustomerJob($job, $translator, $job_id, $response)
    {
...
//                Event::fire(new JobWasCanceled($job));
    ...
    private function cancelCustomerJob($job, $translator, $job_id, $response){
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cUser && $cUser->user_type == config('roles.super_admin')) {
            $allJobs = $this->getAllIfUserIsSuperUser($requestdata, $limit);
        } else {
            $allJobs = $this->getAllIfUserIsNotSuperUser($requestdata, $limit, $consumer_type);
        }
        return $allJobs;
    }

    private function getAllIfUserIsSuperUser($requestdata, $limit){
        $allJobs = Job::query();
    ...
    private function getAllIfUserIsNotSuperUser($requestdata, $limit, $consumer_type) {
        $allJobs = Job::query();
